...
class EventController < ApplicationController
  ...
  def loadSpecificEvent
    ...
    if checkUser
        ...
        specificEvent = Event.find_by_id(params[:event_id])
        if !specificEvent.nil?

          msg[:response] =CodeHelper.CODE_SUCCESS
          msg[:description] = "返回事件成功"
          msg[:event] = specificEvent
          render :json =>  msg
          return
        else
  ...
  def userHasViewedEvent
    ...
    if checkUser
      ...
      event_view.update_attribute(:viewNum, 'true')
    end
  end
  ...
  def getUsersEvents
    ...
    if checkUser
      ...
      else if  params[:isOwnerCheck] = "vistor"
